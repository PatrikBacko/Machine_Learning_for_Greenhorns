...
def main(args: argparse.Namespace) -> tuple[list[tuple[float, float]], float]:
    ...
    target_0 = models[0].predict(test_data)
    target_1 = models[1].predict(test_data)

    for i in range(args.bootstrap_samples):
        indices = generator.choice(len(test_data), size=len(test_data), replace=True)
        scores[0,i] = len(np.where( test_target[indices] == target_0[indices])[0]) / len(test_data) * 100
        scores[1,i] = len(np.where( test_target[indices] == target_1[indices])[0]) / len(test_data) * 100
    ...
    confidence_intervals = [
        (np.percentile(scores[0,:], 2.5), np.percentile(scores[0,:], 97.5)),  # confidence interval of the first model
        (np.percentile(scores[1,:], 2.5), np.percentile(scores[1,:], 97.5)),  # confidence interval of the second model
    ]
    ...
    score = np.zeros([args.bootstrap_samples])
    score = scores[1,:] - scores[0,:]

    result = len(np.where(score <= 0)[0]) / args.bootstrap_samples * 100
