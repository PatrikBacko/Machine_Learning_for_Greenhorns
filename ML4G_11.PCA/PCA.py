...
parser.add_argument("--pca", default=20, type=int, help="PCA dimensionality")
...
class PCATransformer(sklearn.base.TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        ni = 1/X.shape[0] * np.sum(X, axis=0)       #error
        s = 1/X.shape[0] * ((X - ni).T @ (X - ni))  #matrix s
        self._V = np.zeros((X.shape[1], 0)) #eigen vectors matrix
        ...
        if self._n_components <= 10:
            ...
            for i in range(self._n_components):
                ...
                for i in range(10):
                    v = s @ v
                    e_value = np.linalg.norm(v)
                    v = v/e_value
                s = s - (e_value*(np.outer(v, v)))
                self._V = np.concatenate((self._V, v.reshape((v.shape[0],1))), axis= 1)
        else:
            # TODO: Use the SVD decomposition computed with `np.linalg.svd`
            # to find the principal components.
            self._V = (np.linalg.svd(s, compute_uv = True))[0][:,:self._n_components]
