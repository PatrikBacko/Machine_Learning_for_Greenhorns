...
parser.add_argument("--seed", default=49, type=int, help="Random seed")
parser.add_argument("--test_size", default=0.9, type=lambda x: int(x) if x.isdigit() else float(x), help="Test size")
...
def main(args: argparse.Namespace) -> float:
    ...
    target = [models[0].predict(test_data), models[1].predict(test_data)]
    

    for i in range(args.random_samples):
        assignments = generator.choice(2, size=len(test_data), replace=True)

        indices_0 = (np.where(assignments == 0))[0]
        indices_1 = (np.where(assignments == 1))[0]

        sum_0 = len(np.where( test_target[indices_0] == target[0][indices_0])[0])
        sum_1 = len(np.where( test_target[indices_1] == target[1][indices_1])[0])

        scores[i] =  (sum_0 + sum_1 )/ len(test_data) * 100

    second_model = len(np.where( test_target == target[1])[0]) / len(test_data) * 100
    ...
    p_value = len(np.where(scores >= second_model)[0]) / args.random_samples * 100
